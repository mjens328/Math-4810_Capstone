...
```{python}
#| title: Read in Parcel Export Sheet 2
#| cache: true
...
```{python}
#| title: Filter Parcel Export for PCA (only numeric columns)
#| cache: true
...
```{python}
#| title: Scaling the data for PCA
#| cache: true
...
```{python}
#| title: Optimize K for K-Means using Elbow Method
#| cache: true
wcss = []
for i in range(1, 11): 
...
```
```{python}
#| title: K-Means Clustering and Visualization for K=2 to K=5
#| cache: true
for i in range(2,6):
...
```
```{python}
#| title: Relate back to original features for K=2 to K=5
#| cache: true
for i in range (2,6):
    kmeans = KMeans(n_clusters=i, random_state=42, n_init=10)
    cluster_labels = kmeans.fit_predict(pca_df)
